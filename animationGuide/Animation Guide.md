...
Guide version `1.1` updated 11/08/2026.

Quick Access:
- [Everything Blender](#everything-blender)
- [Everything Godot](#everything-godot)
...
> ### Layered Animations
If you intend on layering animations within Godot there are some factors to keep in mind. A layered animation might include:
- Directional tails
- Moving wires
- Moving Hair
- Clothing
...
When creating these types of animations:

- It would be best to **only keyframe necessary bones**. Keying in all bones will very likely cause issues when layering animations within Godot's `AnimationTree`
- If you do keyframe bones you don't need, select them in pose mode and just delete their yellow dots from the keyframe window. **Reset their positions, rotations and scales as needed** before baking.
- Keep them bare-boned (lmao). Making them too complex raises the likelihood of the above point. Especially until you find your individual workflow
- When baking, you should still **bake all bones**.

This advice depends on the workflow you have with animation in Godot. From experience using the `AnimationTree` and the `Add2` nodes, when baking unnecessary bones it caused the armature to deform in unexpected ways.
